...
class RescaleT(object):
    ...
    def __init__(self, output_size):
        # 断言output_size的类型为int或tuple
        assert isinstance(output_size, (int, tuple))
        # 将output_size赋值给self.output_size
...
class SalObjDataset(Dataset):
    def __init__(self, img_name_list, lbl_name_list, transform=None):
        ...
        idx = 0
        while idx < len(img_name_list):
            image = cv2.imread(img_name_list[idx])
            if len(lbl_name_list) == 0:
                label = np.zeros(image.shape)
            else:
                label = cv2.imread(lbl_name_list[idx])
                # img = cv2.merge([label,label, label])  # 转三通道

                label = cv2.cvtColor(label, cv2.COLOR_BGR2GRAY)  # 转灰度图
                label = label[:, :, np.newaxis]
            sample = {"imidx": np.array([idx]), "image": image, "label": label}
            if self.transform:
                sample = self.transform(sample)

            self.img_data_list.append(sample["image"])
            self.label_data_list.append(sample["label"])
    ...
    def __getitem__(self, idx):
        return self.img_data_list[idx], self.label_data_list[idx]
